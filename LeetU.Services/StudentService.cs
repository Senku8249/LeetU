...
public class StudentService : IStudentService
{
    ...
    //новый метод получить ид всех студентов + интерфейс(контроллер)
    public IEnumerable<long> GetAllStudentIds()
    {
        // Берём всех студентов (фильтр true) и возвращаем только Id
        var entities = _studentRepositoryCrud.Get(s => true);

        foreach (var entity in entities)
            yield return entity.Id;
    }

    public IEnumerable<long> GetStudentIdsByCourseId(long courseId)
    {
        // Берём связи StudentCourse по courseId и возвращаем уникальные studentId
        var entities = _studentCourseRepositoryCrud.Get(sc => sc.CourseId == courseId);

        // Distinct на всякий случай, если в таблице могут быть дубли
        foreach (var id in entities.Select(e => e.StudentId).Distinct())
            yield return id;
    }
}